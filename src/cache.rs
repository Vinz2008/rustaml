...
/*pub fn get_cached_ast(rustaml_context : &mut RustamlContext, content : &str) -> Option<FrontendOutput> {
    let cache_dir = cache_dir()?;
    let mut ast_cache_dir = pathbuf![&cache_dir, "rustaml", "ast"];
    if !ast_cache_dir.exists(){
        return None;
    }
    
    let mut hasher = blake3::Hasher::new();
    hasher.write(content.as_bytes()).unwrap();
    let hash = hasher.finalize();
    
    ast_cache_dir.push(&format!("{}.json", hash));
    let cached_file = ast_cache_dir;
    if !cached_file.exists(){
        return None;
    }
    let cached_file_bytes = fs::read(&cached_file).unwrap();
    let serialized_ast = from_bytes::<SerializedAst>(&cached_file_bytes).unwrap();
    rustaml_context.str_interner = serialized_ast.str_interner;
    rustaml_context.ast_pool = serialized_ast.ast_pool;
    rustaml_context.pattern_pool = serialized_ast.pattern_pool;
    let frontend_output = serialized_ast.frontend_output;
    Some(frontend_output)
}

pub fn write_cached_ast(rustaml_context : &RustamlContext, content : &str, frontend_output : FrontendOutput){
    let cache_dir = if let Some(cache_dir) = cache_dir() {
        cache_dir
    } else {
        return;
    ...
    ast_cache_dir.push(&format!("{}.ast", hash));
    let cached_file = ast_cache_dir;
    // TODO : should it do this ?
    if cached_file.exists(){
        return;
    }
    let mut f = File::create(cached_file).unwrap();
    let serialized_ast_before = SerializedAst {
        frontend_output: frontend_output,
        ast_pool: rustaml_context.ast_pool.clone(),
        str_interner: rustaml_context.str_interner.clone(),
        pattern_pool: rustaml_context.pattern_pool.clone(),
    };
    let buf = to_allocvec(&serialized_ast_before).expect("error when serializing ast");
    f.write(&buf).unwrap();
}*/